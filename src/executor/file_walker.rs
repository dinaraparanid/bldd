use std::{collections::HashMap, error::Error, process::{ChildStdout, Command, Stdio}, usize};
...
fn get_entry_info(entry: DirEntry) -> Result<(DirEntry, Option<ElfArch>, Vec<Library>), Box<dyn Error>> {
    if entry.file_type().is_dir() {
        // force traversing directories
        return Ok((entry, None, vec![]))
    }
...
fn get_libs(entry: &DirEntry) -> Result<Vec<Library>, Box<dyn Error>> {
    ...
    let libs_out = String::from_utf8(ldd_out)?;

    let libs = libs_out
        .lines()
        .filter_map(|s| s.split_whitespace().find(|x| !x.is_empty()))
        .map(|x| x.to_string())
        .collect();
